...
2 参数问题
(1) 参数检查：公共方法需要检查参数有效性并抛出异常（包括构造方法）
(2) 参数断言：非公共方法使用断言检查参数有效性(检查代价过高，可进行隐式检查)
(3) 可变参数：审慎地使用可变参数,当不传递参数时候，空数组仍然存在，相当于默认参数为空数组,运行时会创建数组，存在性能问题
(4) 参数个数：拆分参数列表（方法拆分/内部类封装参数/构造者模式）
...
(1) 不要导出具有相同参数量且参数协变的重载(避免混淆/依靠强制转换进行方法选择),方法签名不一致的重载是可以的, 如果重载，那么应该确保在传递相同的参数时，所有重载的行为都是相同的。
...
域注释：字段使用名字短语描述.
方法注释: 方法使用动词短语描述.
方法的文档注释应该简洁地描述方法与其客户端之间的契约: 前置条件+后置条件+副作用+线程文档+序列化文档
