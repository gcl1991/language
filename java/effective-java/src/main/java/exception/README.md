...
2 可控异常：强迫处理或传播，提高可靠性，但增加了负担，违反开闭和封装原则，难以在Stream中使用。
3 不可控异常：RuntimeException（编程错误/前提违例）和Error（资源不足）不需要也不应该被捕获, 属于不可恢复的情形。
4 状态测试方法：不适合并发环境/状态测试代价太高则慎用，可读性好
5 optional返回值：缺点是无法返回任何额外的信息来详细说明它无法执行你想要的计算。
6 可识别的返回值：容易导致忘记检查识别码，引起BUG
