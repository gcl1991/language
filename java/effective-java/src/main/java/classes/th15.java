...
public class th15 {
    ...
    // 3 成员应设计为私有，只有当同一个包中其他类真的需要访问成员时，需要删除私有修饰符变为包私有。如果发现经常这样做，你应该检查类是否需要重新设计
    // 4 除静态变量之外（必须不可变,使用不可变视图或Clone以避免可变对象和数组造成的对象间接逸出），实例字段和静态字段不应是共有或受保护的，
    // 5 用于测试目的，私有成员指定为包级私有是可以接受的，更高则不行。
